-- Migration to standardize all status columns to use ENUM types
-- This script will:
-- 1. Create/update ENUM types with correct values
-- 2. Convert TEXT status columns to ENUM types
...
-- Step 2: Drop existing triggers that depend on status columns
...
-- Step 3: Convert good_receive_notes status column to grn_status enum
...
-- Step 4: Convert sale_invoices status column to invoice_status enum
...
-- Step 5: Convert sales_orders status column to grn_status enum (same as good_receive_notes)
...
SELECT 
  ...
  udt_name
FROM information_schema.columns 
WHERE table_name = 'sale_invoices' AND column_name = 'status'
...
SELECT 
  ...
  udt_name
FROM information_schema.columns 
WHERE table_name = 'good_receive_notes' AND column_name = 'status'; 