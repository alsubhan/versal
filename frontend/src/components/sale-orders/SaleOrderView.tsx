...
export const SaleOrderView = ({ open, onOpenChange, salesOrder }: SaleOrderViewProps) => {
  const { currency } = useCurrencyStore();
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [products, setProducts] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [completeData, setCompleteData] = useState<SalesOrder | null>(null);
  ...
  const loadCompleteData = async () => {
    ...
    try {
      setLoading(true);
      const data = await getSalesOrder(salesOrder.id);
      if (data && !data.error) {
        ...
        // Process items from existing sales order
      ...
      console.error('Error loading complete sales order data:', error);
      // Process items from existing sales order
      const processedItems = (salesOrder.items || []).map(item => {
        const product = products.find(p => p.id === item.productId);
        return {
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Loading Sales Order...</DialogTitle>
          </DialogHeader>
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-center py-8">
            <div className="text-center">
              <p className="text-gray-600">Sales order data could not be loaded.</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>View Sales Order - {completeData.orderNumber}</DialogTitle>
        </DialogHeader>
        
        {/* TOP SECTION - Sales Order Details */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 pt-4">
          <div>
            <Label htmlFor="orderNumber" className="text-sm font-medium text-gray-700">Sales Order #</Label>
            <Input
              id="orderNumber"
