...
import { getCreditNote, getProducts } from "@/lib/api";
...
export const CreditNoteView = ({ open, onOpenChange, creditNote }: CreditNoteViewProps) => {
  const { currency } = useCurrencyStore();
  const [products, setProducts] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [completeData, setCompleteData] = useState<CreditNote | null>(null);
  ...
  const loadCompleteData = async () => {
    ...
    try {
      setLoading(true);
      const data = await getCreditNote(creditNote.id);
      if (data && !data.error) {
        ...
        // Process items from existing credit note
      ...
      console.error('Error loading complete credit note data:', error);
      // Process items from existing credit note
      const processedItems = (creditNote.items || []).map(item => {
        const product = products.find(p => p.id === item.productId);
        return {
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Loading Credit Note...</DialogTitle>
          </DialogHeader>
  ...
  if (!completeData) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Credit Note Not Found</DialogTitle>
          </DialogHeader>
          <div className="flex items-center justify-center py-8">
            <div className="text-center">
              <p className="text-gray-600">Credit note data could not be loaded.</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>View Credit Note - {completeData.creditNoteNumber}</DialogTitle>
        </DialogHeader>
        
        {/* TOP SECTION - Credit Note Details */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 pt-4">
          <div>
            <Label htmlFor="creditNoteNumber" className="text-sm font-medium text-gray-700">Credit Note #</Label>
            <Input
              id="creditNoteNumber"
              value={completeData.creditNoteNumber}
              disabled
          ...
          <div>
            ...
            <Input
              id="customerId"
              value={completeData.customer?.name || 'Unknown Customer'}
              disabled
          ...
          <div>
            <Label htmlFor="creditDate" className="text-sm font-medium text-gray-700">Credit Date</Label>
            <Input
              id="creditDate"
              value={completeData.creditDate ? format(new Date(completeData.creditDate), "PPP") : "N/A"}
              disabled
              className="mt-1 bg-gray-50"
            />
          </div>
